                    ...
                    <form action="{{ route('partners.update', $partner->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="name" id="name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="{{ old('name', $partner->name) }}" required>
                            </div>
                             ...
                             <div>
                                ...
                                <select name="type" id="type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                                    ...
                                    <option value="kerjasama" {{ old('type', $partner->type) == 'kerjasama' ? 'selected' : '' }}>Kerjasama</option>
                                    <option value="media_partner" {{ old('type', $partner->type) == 'media_partner' ? 'selected' : '' }}>Media Partner</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="hidden" name="oldLogo" value="{{ $partner->logo }}">
                                @if ($partner->logo)
                                    <img src="{{ asset('storage/' . $partner->logo) }}" alt="{{ $partner->name }}" class="mt-2 w-48 h-auto object-contain">
                                    <p class="mt-1 text-sm text-gray-500">Kosongkan jika tidak ingin mengganti logo.</p>
                                @endif
                            </div>
                        </div>
